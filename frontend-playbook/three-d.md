...
> **Relation to `motion-templates.md` §10:** §10 provides the reference *code* for category B (the cursor-displaced field). This manual provides the *judgment* for all seven categories. When you reach for §10's code, confirm via §3 that your domain is actually a B domain. If it isn't, §10's code is the wrong starting point — use the category your matrix row names.
